...
def halvesies(numbers):
    ...
    for num in numbers:
        divided_by_two.append(float(num / 2))
...
def reverse_list_in_place(items):
    ...
    **Do not use** the python function `reversed()` or the method
    ...
    For example::
        ...
        ['I', 'love', 'cookies']
    """
    # reverse by splitting length in half, then reassigning num at i to num at length - i - 1
    # length = len(items)
    # half_length = int(len(items) / 2)

    # for i in range(half_length):
    #     num = items[i]
    #     items[i] = items[length - i - 1]
    #     items[length - i - 1] = num


    # recursive solution to reverse a list
    # if len(items) == 1:
    #     return items
    # # Otherwise
    # return reverse_list_in_place(items[1:]) + items[0:1]
...
def duplicates(items):
    ...
    return sorted(duplicates_array)
